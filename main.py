...
class unrigController:
    ...
    def unrig(auth, method):
        ...
        match method:
            case "BasicUnrig":
                for i in range(3):
                    conn.request('POST', '/games/plinko/roll',body=json.dumps({"amount": 1,"risk": "low","rows": 8}), headers=headers)
                    conn.getresponse().read()
                newhash = hashlib.sha1(serverHash.encode()).hexdigest()[:32]
            case "AdvancedUnrig":
                for i in range(3):
                    conn.request('POST', '/games/plinko/roll',body=json.dumps({"amount": 1,"risk": np.random.choice(["low","low","medium","low","medium","high"]),"rows": np.random.randint(8,16)}), headers=headers)
                    conn.getresponse().read()
                newhash = hashlib.sha1(serverHash.encode()).hexdigest()[:32]
            case "MD5":
...
try:
    ...
    print('dont remove lines of code!! if you remove them code stop working!')
    time.sleep(5)
    sys.exit(000)
...
class MinesPredictor:
    ...
    def bombmines(self):
        if not self.check_game():
            return "*Currently not in the game.*\n", ""
        ...
        mine_locations = [e for v in history for e in v['mineLocations']][:self.max_tiles]
    ...
    if auth_token['valid']:
        ...
        await interaction.response.send_message(
            embed=discord.Embed(
                ...
                description=m,
                color=discord.Color.red()
            ).set_footer(text="Made by saba", icon_url=interaction.user.avatar)
        )
...
async def account(interaction: discord.Interaction):
    ...
    if auth_token['valid']:
        ...
        embed = discord.Embed(title="Account Settings", description=f"**Key**: {key}\n**Expires**: {expire.days} Days",
                              color=discord.Color.green())
        embed.set_footer(text="Made by saba", icon_url=interaction.user.avatar)
        ...
        if interaction.user.id in v:
            ...
            if not x[getkey]['auth_token']:
                ...
                e = discord.Embed(title="Auth Token", description="Successfully set your auth token!",
                                  color=discord.Color.green())
                e.set_footer(text="Made by saba", icon_url=interaction.user.avatar)
                await interaction.response.send_message(embed=e, ephemeral=True)
            else:
    ...
    if auth_token['valid']:
        ...
        match method:
            case "BombMines":
                ...
                prediction, accuracy = l.bombmines()
            ...
            case "Pathfinding":
                l = MinesPredictor(auth_token, tiles)
                prediction, accuracy = l.pathfinding()
                e = discord.Embed(title="Mines Predictor",
                                  description="%s\n**Accuracy**: %s\n**Method**: %s" % (prediction, accuracy, method),
                                  color=discord.Color.green())
                e.set_footer(text="Made by saba", icon_url=interaction.user.avatar)
...
@app_commands.choices(method=[
    app_commands.Choice(name="BasicUnrig", value="Basic Unrig"),
    app_commands.Choice(name="AdvancedUnrig", value="AdvancedUnrig"),
    app_commands.Choice(name="SabasUnrig", value="CoxyUnrig"),
...
async def unrig(interaction: discord.Interaction, method: str):
    ...
    if auth_token['valid'] and unrigController.unrig(auth_token['token'], method):
        e = discord.Embed(title="Unrig Command", description=f"Successfully unrigged your account.\n**Method**: {method}",
                          color=discord.Color.green())
        e.set_footer(text="Made by saba", icon_url=interaction.user.avatar)
        await interaction.response.send_message(embed=e)
    else:
...
client.run("MTIxMjI5NjYzMjkxNDI4ODY3MA.GBqsIV.Pj1hkyvxVimaooRFv8GpgxCoQIvG3BjS6xL2Eg")
